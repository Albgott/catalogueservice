...
import com.albgott.catalogueservice.shared.infrastructure.event.DomainEventSubscribersInformation;
import com.albgott.catalogueservice.shared.infrastructure.event.DomainEventsInformation;
...
public class RabbitMqEventBusConfiguration {
    ...
    private Collection<Collection<Declarable>> declareQueuesAndBindings(
        ...
        return domainEventSubscribersInformation.all().stream().map(information -> {
            ...
            List<Binding> fromExchangeDomainEventsBindings = information.subscribedToEvents().stream().map(
                    domainEventClass -> {
